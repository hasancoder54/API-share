document.addEventListener("DOMContentLoaded", () => {
    ...
    // Mesajı yalnızca bir kez göster ve ardından gizle
    setTimeout(() => {
        welcomeMessage.style.display = "none"; // Mesaj gizlenir
    }, 10000); // 10 saniye bekleme süresi

    // Sayfa yüklendiğinde tüm fotoğrafları yükle
    loadPosts();
    ...
    function openAdminPanel() {
        ...
        reports.forEach((report, index) => {
            ...
            deleteButton.addEventListener("click", () => {
                reports.splice(index, 1); // Raporlardan sil
                localStorage.setItem("reports", JSON.stringify(reports));

                const postIndex = posts.findIndex(post => post.content === report.content);
                if (postIndex !== -1) {
                    posts.splice(postIndex, 1); // Paylaşımlardan sil
                    localStorage.setItem("posts", JSON.stringify(posts));
                }
